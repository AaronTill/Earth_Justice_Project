...
cancer_points_generous_rings <- ggmap(LA_map) +
  ...
  geom_point(data = tidy_data[0:1,], aes(denka_lng, denka_lat), alpha = 1,shape = 21, size = 92)+
  geom_point(data = tidy_data[0:1,], aes(denka_lng, denka_lat), alpha = 1, shape = 21, size = 135)+
  geom_point(data=tidy_data, aes(hh_add_lng, hh_add_lat, color = cancer, alpha = cancer), size = 0.5) +
...
cancer_zone_data <- tidy_data %>%
  ...
  summarise(cancer_after_proportion = sum(na.omit(cancer_after))/n(), cancer_proportion = sum(na.omit(cancer))/n(),  n())
